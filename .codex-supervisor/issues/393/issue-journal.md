...
- Next exact step: Commit the extracted board/task-detail slice checkpoint and open or update a draft PR for `codex/issue-393` if one does not already exist.
...
- Commands run: `pnpm install`; `pnpm --filter @atlaspm/web-ui test -- --run src/feature-slices.test.ts`; `pnpm --filter @atlaspm/web-ui type-check`; `pnpm --filter @atlaspm/web-ui lint`; `pnpm --filter @atlaspm/web-ui test`.
### Scratchpad
