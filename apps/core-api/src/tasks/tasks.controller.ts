...
export class TasksController {
  ...
  async create(@Param('id') projectId: string, @Body() body: CreateTaskDto, @CurrentRequest() req: AppRequest) {
    ...
    const status = this.domain.deriveStatusForProgress(progress, requestedStatus);
  ...
  async patch(@Param('id') id: string, @Body() body: PatchTaskDto, @CurrentRequest() req: AppRequest) {
    ...
    const progressAutomation = this.domain.deriveTaskProgressAutomation(progress, task.status, task.completedAt);
    const status = body.status ?? progressAutomation.status;
    const completedAt = status === TaskStatus.DONE ? task.completedAt ?? progressAutomation.completedAt ?? new Date() : null;
