...
import { AlertTriangle, Calendar, Users, Briefcase } from 'lucide-react';
    ...
    <div className="container mx-auto py-8">
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {workload.map((user) => (
            <UserWorkloadCard key={user.userId} workload={user} />
          ))}
        </div>
      )}
    </div>
...
function UserWorkloadCard({ workload }: { workload: import('@/lib/api/workload').UserWorkload }) {
  const hasOverloads = workload.overloadAlerts.length > 0;
  const maxTasks = Math.max(...workload.weeklyBreakdown.map((w) => w.taskCount), CAPACITY);
      ...
      <CardContent>
        <div className="space-y-4">
          {workload.weeklyBreakdown.map((week) => {
            const isOverloaded = week.taskCount > CAPACITY;
              ...
              <div key={week.week} className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    {isOverloaded && (
                      <Badge variant="destructive" className="text-xs">
                        +{week.taskCount - CAPACITY} over capacity
                      </Badge>
                    )}
                    <span
                      className={cn(
                ...
                <div className="relative h-2 bg-muted rounded-full overflow-hidden">
                  ...
                  <div
                    ...
                    style={{ left: `${(CAPACITY / maxTasks) * 100}%` }}
                  />
                </div>
