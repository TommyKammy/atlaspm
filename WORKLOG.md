...
## 2026-02-27 - P4-2 (Wave 2) Collab Snapshot No-op Suppression
- What changed:
  - Implemented no-op suppression for collab snapshot saves in:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/core-api/src/collab/collab.controller.ts`
  - New behavior in `POST /tasks/:id/description/snapshot`:
    - compares incoming `descriptionDoc` + normalized `descriptionText` against current task snapshot
    - if unchanged, returns `{ ok: true, noop: true, descriptionVersion: <current> }`
    - skips DB update, mention sync, AuditEvent append, and OutboxEvent append
    - logs `collab.snapshot.noop` for observability.
  - Added stable JSON comparison helper (key-sorted normalization) to avoid false negatives from key-order differences.
  - Added integration assertions in:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/core-api/test/core.integration.test.ts`
    - verifies second identical snapshot does not emit `task.description.snapshot_saved` audit/outbox and keeps version unchanged.
- Why:
  - Prevent activity/audit noise from interval/idle/disconnect overlap when no actual description change occurred.
  - Keep collab snapshot events meaningful while preserving durability behavior for real edits.
- How tested (exact commands):
  - `pnpm --filter @atlaspm/core-api lint`
  - `pnpm --filter @atlaspm/core-api type-check`
  - `pnpm --filter @atlaspm/core-api test`
  - `pnpm e2e` (32 tests passed)
- Risks/known gaps:
  - No-op判定は `descriptionDoc + descriptionText` ベース。将来 snapshot payload に追加される意味的フィールドを監査対象にする場合は比較条件を拡張する必要あり。

## 2026-02-27 - P4-2 (Wave 3) Collab Interval Snapshot Noise Reduction
- What changed:
  - Tuned collab-server snapshot cadence to avoid early partial interval saves during initial typing:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/collab-server/src/index.ts`
  - Room state now tracks `lastChangedAt`.
  - On first change in a room, `lastSavedAt` is initialized to the change timestamp (prevents immediate interval save before idle save).
  - Interval snapshot loop now skips rooms with very recent edits (`< 3s`) to let idle/disconnect snapshot settle.
- Why:
  - Prevent patterns like `tes` (interval) then `test` (idle) being logged as two rapid activity entries for a single short edit.
  - Keep interval durability for long-running edits while reducing short-edit activity noise.
- How tested (exact commands):
  - `pnpm --filter @atlaspm/collab-server lint`
  - `pnpm --filter @atlaspm/collab-server type-check`
  - `pnpm --filter @atlaspm/collab-server build`
  - `pnpm e2e` (32 tests passed)
- Risks/known gaps:
  - Long continuous edits can still produce multiple snapshot events by design (periodic durability); this change targets noisy near-immediate duplicates for short edits.

## 2026-02-27 - P4-2 (Wave 4) Concurrent Snapshot Dedup (CAS)
- What changed:
  - Hardened collab snapshot save path against concurrent duplicate writes:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/core-api/src/collab/collab.controller.ts`
  - `POST /tasks/:id/description/snapshot` now uses CAS-style update:
    - reads current `descriptionVersion` in transaction
    - no-op check against current doc/text
    - updates via `updateMany(where: { id, descriptionVersion: currentVersion })`
    - if claim fails, retries up to 3 times
    - on retry path, second concurrent writer re-evaluates and typically resolves to `noop`.
  - Prevents duplicate `task.description.snapshot_saved` audit/outbox rows when idle/disconnect requests arrive concurrently with same content.
  - Extended integration test:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/core-api/test/core.integration.test.ts`
    - fires two concurrent snapshot requests with identical payload
    - asserts exactly one snapshot outbox event and one snapshot audit event across both correlationIds.
- Why:
  - Close remaining race where two near-simultaneous snapshot requests could both pass pre-check and emit duplicate activity entries.
- How tested (exact commands):
  - `pnpm --filter @atlaspm/core-api lint`
  - `pnpm --filter @atlaspm/core-api type-check`
  - `pnpm --filter @atlaspm/core-api test`
  - `pnpm e2e` (32 tests passed)
- Risks/known gaps:
  - If collab traffic spikes with sustained distinct edits, multiple snapshot events remain expected by design (durability cadence), but same-content duplicate emissions are now suppressed for both sequential and concurrent paths.

## 2026-02-27 - P4-2 (Wave 5) Collab Snapshot Dispatch Dedup + Activity Compaction
- What changed:
  - Added room-level snapshot dispatch serialization in collab-server to prevent overlapping `idle/interval/disconnect` saves:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/collab-server/src/index.ts`
    - added `saving` lock + `pendingReason` coalescing + disconnect debounce cancellation.
  - Added same-content key suppression in collab-server:
    - stores `lastSavedSnapshotKey` (normalized doc + text)
    - skips API call when next snapshot content is identical.
  - Added UI-side compaction for near-duplicate snapshot activity rows:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/web-ui/src/components/task-detail-drawer.tsx`
    - compresses consecutive `task.description.snapshot_saved` rows from `collab-server` when the latter event is effectively no-op and near-immediate.
- Why:
  - Eliminate remaining duplicate-looking snapshot history noise caused by event timing collisions and reconnect edges.
  - Keep audit durability behavior while preventing low-value repeated timeline entries.
- How tested (exact commands):
  - `pnpm --filter @atlaspm/collab-server lint`
  - `pnpm --filter @atlaspm/collab-server type-check`
  - `pnpm --filter @atlaspm/collab-server build`
  ...
  - `pnpm e2e` (32 tests passed)
- Risks/known gaps:
  - UI compaction intentionally affects only specific near-duplicate snapshot sequences; non-identical edits remain visible as separate events.

## 2026-02-27 - P4-2 (Wave 6) Snapshot Actor Attribution to Real Editor
- What changed:
  - Added actor propagation from collab-server to core-api snapshot endpoint:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/collab-server/src/index.ts`
      - snapshot payload now includes `actorUserId` (last editor), sourced from context and participant fallback.
    - `/Users/tomoakikawada/Dev/atlaspm/apps/core-api/src/collab/collab.controller.ts`
      - `SnapshotBodyDto` now accepts optional `actorUserId`
      - resolves actor safely against project membership (`resolveSnapshotActor`)
      - writes audit/outbox/mention-notification actor as resolved user, or falls back to `collab-server`.
  - Expanded integration coverage:
    - `/Users/tomoakikawada/Dev/atlaspm/apps/core-api/test/core.integration.test.ts`
      - valid member actor is recorded for snapshot events
      - invalid actor id falls back to `collab-server`.
- Why:
  - Make Activity timeline trustworthy by showing who actually edited description, while keeping service-side fallback and security checks.
- How tested (exact commands):
  - `pnpm --filter @atlaspm/collab-server lint`
  - `pnpm --filter @atlaspm/collab-server type-check`
  - `pnpm --filter @atlaspm/collab-server build`
  ...
  - manual DB verification:
    - snapshot audit/outbox now stores `actor=dev-user-1` for valid member actorUserId.
- Risks/known gaps:
